...
export default function ListPage() {
    ...
    const handleDeleteBook = async (book) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/${book.id}`, { method: 'DELETE' });
                        ...
                        <div className="filter-bar mb-3">
                            <div className="d-flex flex-wrap gap-2 align-items-center justify-content-between">
                                ...
                                <div className="d-flex gap-2">
                                    <select className="form-select form-select-sm" style={{ width: 'auto' }} value={currentSort} onChange={(e) => setCurrentSort(e.target.value)}>
                            ...
                            <table className="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="text-end" style={{ width: '200px' }}>관리</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {isLoading ? (
                                        ...
                                        displayedBooks.map(book => (
                                            <tr key={book.id}>
                                                ...
                                                <td className="text-end">
                                                    <button className="btn btn-sm btn-outline-info" onClick={() => navigate(`/detail/${book.id}`)}>
                                                        상세
                                                    </button>
                                                    <button className="btn btn-sm btn-outline-primary ms-1" onClick={() => navigate(`/update/${book.id}`)}>
                                                        수정
                                                    </button>
